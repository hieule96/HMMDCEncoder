...
class Optimizer_curvefitting(Optimizer):
    ...
    nbCoeff = 0
    def __init__(self,oparam=None,curve_fitting_function = "exp"):
    ...
    @staticmethod
    def searchForQPbyRate(img,node,rtarget):
        # Take image in partition and dct
    ...
    def curve_fitting(self,LCU,node):
        ...
        if (not node.isSkip):
            try:
                ...
                a,b,r,_,_ = scipy.stats.linregress(entropyQP,log_mse_QP)
                DRcoeff=[np.exp(b),a,0]
    ...
    def optimizeQuadtreeLambaCst(self, qtree,lam1,lam2,mu1,mu2,Ci1,Ci2,Dm,rN):
        ...
        for node in qtree:
            if (node.isSkip):
                ...
                a,b = Optimizer_curvefitting.searchForQPbyRate(self.globalparam.LCU.img,node,r1[r_count])
                c,d = Optimizer_curvefitting.searchForQPbyRate(self.globalparam.LCU.img,node,r2[r_count])
                QP1.append(a)
                entropy1.append(b)
                QP2.append(c)
                entropy2.append(d)
